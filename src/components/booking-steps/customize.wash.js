...
export function CustomizeWash(props) {
  useEffect(() => {
  ...
  const handleExtraCount = (extra, variant) => {
    const val = props.scheduleInfo[extra];
    if (variant === "add") return props.changePDInfo(extra, val + 1);
    if (variant === "minus" && val > 0)
      return props.changePDInfo(extra, val - 1);
  };

  const handleWashCount = (variant) => {
    if (variant === "add") {
      props.changePDInfo("washCount", (props.scheduleInfo.washCount || 0) + 1);
      return props.changePDInfo(
        "softener",
        (props.scheduleInfo.softener || 0) + 1
      );
    }
  ...
  return (
    <div className='step-view customize-wash'>
      ...
      <div className='__options'>
        <div className='option'>
          ...
          <div className='wash-counter-flex'>
            ...
            <h3>
              N{WASH_PRICES.WASH} <span>per wash </span>
            </h3>
          </div>
        </div>
      </div>
